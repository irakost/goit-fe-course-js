...
//     return arr.filter(function(user, i, arr){
...
//  */
 const getUsersWithAge = (arr, min, max) => arr.filter(user => min <= user.age && user.age  <= max);
...
*/
const getUserNamesSortedByFriendsCount = arr => arr.sort((a, b) => a.friends.length - b.friends.length)
                                                   .map(user => user.name);
